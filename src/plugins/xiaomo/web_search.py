...
_EXPLICIT_TRIGGERS = [
    r"(?:搜索|搜一下|搜一搜|查一下|查一查|查查)(?:一下|一哈)?\s*(.+)",
    r"(?:帮我|给我|麻烦)(?:搜索|搜|查)(?:一下|一哈)?\s*(.+)",
    r"上网(?:搜索|搜|查)(?:一下|一哈)?\s*(.+)",
    r"联网(?:搜索|搜|查)(?:一下|一哈)?\s*(.+)",
    r"网络搜索(?:一下|一哈)?\s*(.+)",
    r"网上搜(?:一下|一哈)?\s*(.+)",
...
_NATURAL_CANDIDATES = [
    r"(?:现在|最近|今日|今天|明天|昨天|本周|这个月|今年).{0,30}(?:新闻|热搜|热榜|价格|行情|比分|比赛|赛程|时间|结果|版本|更新|发布|动态|政策|规定|事件)",
    r"(?:新闻|热搜|热榜|头条|价格|行情|比分|赛程|版本|更新|发布|动态).{0,20}(?:现在|最近|今日|今天|最新|多少|什么)",
    r"(?:最新|刚刚|实时).{0,30}(?:版本|新闻|消息|动态|结果|比分|价格|行情|政策|规定|发布|更新|事件)",
    r".{2,30}(?:最新版本|新版本|刚发布|刚更新|今日新闻|实时消息)",
]
...
def is_natural_candidate(text: str) -> bool:
    """Return true only when an answer is likely to require live data."""
